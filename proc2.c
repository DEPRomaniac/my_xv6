...
struct {
  ...
  int lock_type;
  // struct proc proc[NPROC];
  int reader_pid[NPROC];
  int writer_pid[NPROC];
...
void add_reader(int rindex){
  struct proc *curproc = myproc();
  acquire(&rw_mutex.mutex);
  rw_mutex.reader_pid[rw_mutex.rnum++] = curproc->pid;
  cprintf("pid = %d bye bye goto sleep..\n", curproc->pid);
  release(&rw_mutex.mutex);

  while (rw_mutex.rindex < rindex);
  // acquire(&rw_mutex.mutex);
  // sleep(curproc, &ptable.lock);
  if (!rw_mutex.lock.locked){
    // acquire()
  }

  if (rw_mutex.lock.locked && rw_mutex.lock_type == W_LOCK){}
  // cprintf("pid = %d has woken!!\n", curproc->pid);
  // release(&rw_mutex.mutex);
}
...
void add_writer(int windex){
  struct proc *curproc = myproc();
  acquire(&rw_mutex.mutex);
  rw_mutex.writer_pid[rw_mutex.wnum++] = curproc->pid;
  cprintf("pid = %d bye bye goto sleep..\n", curproc->pid);
  release(&rw_mutex.mutex);

  while (rw_mutex.windex < windex);
  // sleep(curproc, &ptable.lock);
...
struct proc* find_proc(uint process_id){
  ...
  if (process_avail == 0)
  {
    cprintf("find_proc ERROR: process with pid=%d does not exist\n", process_id);
    return 0;
  } 
  return curproc;
}